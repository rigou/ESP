...
class rgParam {
	public:
		...
		void SetSeparator(char sep_chr); // FIELDSEP by default
		void End(void);
	...
	private:
		static const char FIELDSEP='\t'; // default
		char mSeparator=FIELDSEP; // FIELDSEP by default, call SetSeparator() to change it
		char *mBuffer_str=NULL; // general purpose buffer, its size is defined by the RGPARAM_BUFFER_LEN macro
		char *mPath_str=NULL; // absolute path
		char **mKeys_str=NULL;
		char **mValues_str=NULL;

		void set_current_path(const char *path_str);
